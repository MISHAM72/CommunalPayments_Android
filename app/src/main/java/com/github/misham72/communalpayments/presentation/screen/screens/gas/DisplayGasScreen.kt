...
fun DisplayGasScreen(viewModel: GasViewModel) {
    ...
    Column(
        ...
        ServiceTopBar(title = uiState.customServiceName.ifBlank { stringResource(R.string.service_display_name_gas) }, onEditClick = { viewModel.openAccountDialog() }, onShareClick = {
            scope.launch {
        ...
        uiState.result?.let { result ->
            Card(
                ...
                Column(
                    ...
                    Text(
                        text = stringResource(R.string.result_gas), style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold
                    )
                    Text(stringResource(R.string.consumption, result.consumption, R.string.unit_cubic_meter))
                    Text(
