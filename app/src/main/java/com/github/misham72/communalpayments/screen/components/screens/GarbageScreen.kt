...
import com.github.misham72.communalpayments.logic.Garbage

@Composable
fun DisplayGarbageScreen() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Button(
            onClick = {
                newGarbageData = newGarbage.collectGarbageData()
                newGarbage.saveGarbageData(newGarbageData ?: return@Button)
        ...
        if (newGarbageData != null) {
            val data = newGarbageData ?: return@Column
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(16.dp)) {
                    Text(stringResource(R.string.result_garbage), fontWeight = FontWeight.Bold)
                    Text(context.getString(R.string.date, fileManager.getCurrentDateTime()))
