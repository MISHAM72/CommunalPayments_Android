...
import com.github.misham72.communalpayments.logic.MTS


@Composable
fun DisplayMTSScreen() {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        Button(
            onClick = {
                newMTSData = newMTS.collectMTSData()
                newMTS.saveMTSData(newMTSData ?: return@Button)
        ...
        if (newMTSData != null) {
            val data = newMTSData ?: return@Column
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(modifier = Modifier.padding(16.dp)) {
                    Text(stringResource(R.string.result_mts), fontWeight = FontWeight.Bold)
