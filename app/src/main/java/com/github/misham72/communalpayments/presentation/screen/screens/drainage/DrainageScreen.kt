...
@Composable
fun DrainageScreen(viewModel: DrainageViewModel, appContainer: AppContainer) {
    ...
    val receiptsViewModelFactory = ReceiptsViewModelFactory(
        appContainer.getReceiptsUseCase,
        appContainer.deleteReceiptUseCase,
        appContainer.saveReceiptUseCase
    )
    ...
    if (showReceipts) {
        ReceiptsScreen(
            serviceKey = DrainageViewModel.SERVICE_KEY,
        ...
        Column(
            ...
            ServiceTopBar(
                title = uiState.providerDetails.customServiceName.ifBlank {
                    serviceName(ServiceKeys.DRAINAGE)
                },
            ...
            Row(
                ...
                Checkbox(
                    checked = uiState.hasHotWater,
                    onCheckedChange = { viewModel.toggleHasHotWater(it) }
                )
                Text(
                    text = stringResource(R.string.has_hot_water),
                    fontSize = 16.sp
                )
            }

            // --- Предупреждения, если нет данных ---
            if (!uiState.hasColdData) {
                Text(
                    text = stringResource(R.string.no_cold_water_data),
                    color = Color.Red,
                    fontSize = 14.sp
                )
            }
            if (uiState.hasHotWater && !uiState.hasHotData) {
                Text(
                    text = stringResource(R.string.no_hot_water_data),
                    color = Color.Red,
                    fontSize = 14.sp
                )
            }
            ...
            // --- Расход ГВС (если флаг) ---
            if (uiState.hasHotWater) {
                Text(
                    text = stringResource(
                        R.string.hot_water_usage,
                        uiState.hotUsage,
                        stringResource(R.string.unit_cubic_meter)
                    ),
                    fontSize = 16.sp,
            ...
            // --- Кнопка "Рассчитать" ---
            val canCalculate = uiState.hasColdData && (!uiState.hasHotWater || uiState.hasHotData)
            ...
            Card(
                ...
                if (result != null) {
                    Column(
                        ...
                        Text(
                            text = stringResource(R.string.drainage_result_title),
                        ...
                        Row(
                            ...
                            IconButton(
                                onClick = {
                                    ...
                                    Toast.makeText(
                                        context,
                                        context.getString(R.string.amount_copied, result.payment),
                                        Toast.LENGTH_SHORT
                                    ).show()
                                },
