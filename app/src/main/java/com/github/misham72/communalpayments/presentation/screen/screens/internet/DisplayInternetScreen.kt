...
fun DisplayInternetScreen(viewModel: InternetViewModel) {
    ...
    Column(
        ...
        ServiceTopBar(
            onPdfExport = { viewModel.onPdfExport(context) }, title = uiState.providerDetails.customServiceName.ifBlank { stringResource(R.string.service_display_name_internet) }, onEditClick = { viewModel.openAccountDialog() }, onShareClick = {
                scope.launch {
                    HistoryExporter.shareSingleHistory(context, InternetViewModel.SERVICE_KEY)
                }
        ...
        AlertDialog(onDismissRequest = { showProviderDialog.value = false }, title = { Text(stringResource(R.string.select_details_to_copy)) }, text = {
            Column {
                ...
                Text(
                    ...
                    style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(bottom = 4.dp)
                )
                // 5. Лицевой счёт
                TextButton(
                    onClick = {
                ...
                TextButton(
                    onClick = {
                        if (uiState.providerDetails.bankAccount.isNotBlank()) {
                            ...
                            Toast.makeText(context, R.string.bank_account, Toast.LENGTH_SHORT).show()
                        } else {
