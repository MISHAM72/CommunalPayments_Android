...
// Номер под названием (отдельная строка)
        if (uiState.accountNumber.isNotBlank()) {
            Text(
                text = stringResource(R.string.contract_number, uiState.accountNumber), fontSize = 14.sp, color = MaterialTheme.colorScheme.error, modifier = Modifier.padding(top = 1.dp)
            )
        }
        ...
        uiState.result?.let { result ->
            ...
            Button(
                onClick = {
                    ...
                    Toast.makeText(
                        context, context.getString(R.string.amount_copied, result.priceTariff),
                        Toast.LENGTH_SHORT
                    ).show()
                },
                modifier = Modifier.fillMaxWidth()
