...
fun DisplayTinkoffScreen(viewModel: TinkoffViewModel) {
    ...
    Column(
        ...
        ServiceTopBar(title = uiState.customServiceName.ifBlank { stringResource(R.string.service_display_name_tinkoff) }, onEditClick = { viewModel.openAccountDialog() }, onShareClick = {
            scope.launch {
...
// Номер под названием (отдельная строка)
        if (uiState.accountNumber.isNotBlank()) {
            Text(
                text = stringResource(R.string.number_phone, uiState.accountNumber), fontSize = 14.sp, color = Color.Red, modifier = Modifier.padding(top = 4.dp)
            )
        }
